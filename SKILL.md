...
```
Create `~/.claude/brain/BRAIN.md` if missing:
```markdown
# BRAIN — Global Index
> One line per entry, pointer to the detail file. HARD LIMIT: 60 lines.
...
grep -q "CONFLICT" $M/primer.md 2>/dev/null && echo "⚠ Unarbitrated memory conflicts in primer"
S=$(ls $M/scratch/*.md 2>/dev/null | wc -l); [ "$S" -gt 0 ] && echo "📋 $S unconsolidated scratch dump(s)"
STALE=$(grep -c "⏳" ~/.claude/brain/BRAIN.md 2>/dev/null || echo 0)
[ "$STALE" -gt 0 ] && echo "⏳ $STALE stale knowledge entries in global brain"
echo "=== END BRIEF ==="
```
...
- `learn.md` → "Promote to global brain: $ARGUMENTS. Write/update an L1 note in ~/.claude/brain/knowledge/<domain>/ or practices/ with full frontmatter (incl. tags synonyms, uses: 1, last_used: today), add its L2 line to BRAIN.md (respect 60-line limit — merge, or evict the lowest-uses/oldest/lowest-confidence line if full)."
...
### Size discipline (hard limits — enforce, don't apologize)
primer ≤150 lines · CLAUDE.md ≤200 · BRAIN.md ≤60 · knowledge note ≤1 page. Overflow moves DOWN the gradient (L2→L1→L0), never accumulates in hot files.
...
- *What does it cost in context?* Hot load is L2 only: BRAIN.md (≤60 lines) + primer (≤150) + brief (~10) ≈ 1–2k tokens. Depth (L1/L0) loads only on demand. The hard limits exist precisely to keep this bounded.
...
- **Hard line limits (60/150/200) force compression** — that's what keeps memory USEFUL. A memory that grows unbounded stops being read; overflow moves down the gradient instead.
